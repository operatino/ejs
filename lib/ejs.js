...
var fs = require('fs')
  ...
  , _REGEX_STRING = '(<%%|<%=|<%-|<%_|<%#|<%|%>|-%>|_%>)'
  , _OPTS = [ 'cache', 'filename', 'sandbox', 'delimiter', 'scope', 'context'
...
function isRestrictedReadPath(pathToRead, options) {
...
function readFile(pathToRead, options) {
  if (options.sandbox && isRestrictedReadPath(pathToRead, options)) {
    throw new Error('reading files beyond sandbox is restricted, limit set to ' + options.sandbox);
  }

  return fs.readFileSync(pathToRead);
}
...
function handleCache(options, template) {
  ...
  if (options.cache) {
    ...
    if (!hasTemplate) {
      template = readFile(path, options).toString().replace(_BOM, '');
    }
  }
  else if (!hasTemplate) {
...
function includeSource(path, options) {
  ...
  template = readFile(includePath, options).toString().replace(_BOM, '');
